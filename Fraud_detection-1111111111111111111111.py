...
增加密码保护功能 - 使用Streamlit Secrets管理
"""
...
def check_password():
    ...
    def password_entered():
        """检查用户输入的密码是否正确"""
        try:
            # 从Streamlit Secrets中获取存储的密码
            if "password" in st.secrets:
                stored_password = st.secrets["password"]
            else:
                # 默认密码，如果没有设置secrets
                stored_password = "biomass2023"
                st.warning("警告: 未配置密码secrets，使用默认密码")
                
            if st.session_state["password"] == stored_password:
                st.session_state["password_correct"] = True
                del st.session_state["password"]  # 验证后删除密码
            else:
                st.session_state["password_correct"] = False
                st.error("密码不正确")
        except Exception as e:
            st.error(f"验证密码时出错: {str(e)}")
            st.session_state["password_correct"] = False
    ...
    if not st.session_state["password_correct"]:
        ...
        st.markdown("<h1 style='text-align: center;'>🔥 生物质热解产物预测系统</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center;'>这是一个受密码保护的应用程序</p>", unsafe_allow_html=True)
        st.text_input(
            "请输入访问密码", 
            type="password", 
            key="password",
        ...
        # 添加页脚
        footer = """
        <div style='text-align: center; position: fixed; bottom: 10px; left: 0; right: 0;'>
...
class CorrectedEnsemblePredictor:
    ...
    def extract_training_ranges(self):
        ...
        if self.metadata and 'feature_ranges' in self.metadata:
            ...
            log(f"从元数据加载了训练数据的真实范围，共 {len(self.training_ranges)} 个特征")
            
            # 记录特征范围到日志
            for feature, range_info in self.training_ranges.items():
                log(f"特征 {feature} 训练范围: {range_info['min']:.2f} - {range_info['max']:.2f}")
