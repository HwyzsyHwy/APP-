...
if st.session_state.current_page == "模型信息":
    ...
    /* 第一列按钮 - 青绿色 - 最高优先级 */
    div[data-testid="column"]:nth-of-type(1) button[aria-label="Increment"],
    div[data-testid="column"]:nth-of-type(1) button[aria-label="Decrement"],
    div[data-testid="column"]:first-child button[aria-label="Increment"],
    div[data-testid="column"]:first-child button[aria-label="Decrement"],
    .stColumn:first-child button[aria-label="Increment"],
    .stColumn:first-child button[aria-label="Decrement"] {
        background: #20b2aa !important;
        background-color: #20b2aa !important;
        background-image: none !important;
    ...
    /* 第二列按钮 - 金黄色 - 最高优先级 */
    div[data-testid="column"]:nth-of-type(2) button[aria-label="Increment"],
    div[data-testid="column"]:nth-of-type(2) button[aria-label="Decrement"],
    .stColumn:nth-of-type(2) button[aria-label="Increment"],
    .stColumn:nth-of-type(2) button[aria-label="Decrement"] {
        background: #daa520 !important;
        background-color: #daa520 !important;
        background-image: none !important;
    ...
    /* 第三列按钮 - 橙红色 - 最高优先级 */
    div[data-testid="column"]:nth-of-type(3) button[aria-label="Increment"],
    div[data-testid="column"]:nth-of-type(3) button[aria-label="Decrement"],
    .stColumn:nth-of-type(3) button[aria-label="Increment"],
    .stColumn:nth-of-type(3) button[aria-label="Decrement"] {
        background: #cd5c5c !important;
        background-color: #cd5c5c !important;
        background-image: none !important;
    ...
    # 使用更强力的CSS和JavaScript组合来强制修改expander标题颜色
    st.markdown("""
    <style>
    /* 使用CSS变量和更高优先级的选择器 */
    ...
    /* 使用属性选择器强制覆盖 */
    [data-testid="stExpander"][style*="color"] summary,
    [data-testid="stExpander"] summary[style*="color"] {
        color: white !important;
    }
    </style>

    <script>
    // 使用MutationObserver持续监控DOM变化
    function forceExpanderWhiteText() {
        ...
        expanders.forEach(function(expander) {
            ...
            if (summary) {
                ...
                summary.style.setProperty('text-shadow', '1px 1px 2px rgba(0,0,0,0.8)', 'important');
                ...
                allElements.forEach(function(el) {
                    ...
                    el.style.setProperty('text-shadow', '1px 1px 2px rgba(0,0,0,0.8)', 'important');
                });
                ...
                while (node = walker.nextNode()) {
                    if (node.parentElement) {
                        ...
                        node.parentElement.style.setProperty('text-shadow', '1px 1px 2px rgba(0,0,0,0.8)', 'important');
                    }
                }
            }
        });
    ...
    // 立即执行
    setTimeout(forceExpanderWhiteText, 100);
    setTimeout(forceExpanderWhiteText, 500);
    setTimeout(forceExpanderWhiteText, 1000);

    // 设置MutationObserver持续监控
    const observer = new MutationObserver(function(mutations) {
        forceExpanderWhiteText();
    });
