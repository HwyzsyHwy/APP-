...
# 记录启动日志
log("Cd2+吸附预测应用启动 - XGBoost模型版本")
log("已加载Cd2+的吸附容量预测模型")
...
class ModelPredictor:
    ...
    def _find_model_file(self):
        """查找模型文件 - 更新后的版本"""
        # Cd2+模型文件名
        filenames = ["XGBoost-Cd2+-model.joblib"]
...
if st.session_state.prediction_result is not None:
    ...
    # 显示主预测结果
    result_container.markdown(f"<div class='yield-result'>Cd2+—AC: {st.session_state.prediction_result:.2f} mg/g</div>", unsafe_allow_html=True)
