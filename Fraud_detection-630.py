...
with col_center:
    ...
    with col1:
        ...
        for feature in feature_categories["Proximate Analysis"]:
            ...
            features[feature] = st.number_input(
                feature, value=float(value), step=0.01, format="%.3f", key=f"prox_{feature}"
            )
    
    with col2:
        st.markdown('<div class="section-title ultimate">Ultimate Analysis</div>', unsafe_allow_html=True)
        for feature in feature_categories["Ultimate Analysis"]:
            value = st.session_state.feature_values.get(feature, default_values[feature])
            features[feature] = st.number_input(
                feature, value=float(value), step=0.001, format="%.3f", key=f"ult_{feature}"
            )
    
    with col3:
        st.markdown('<div class="section-title pyrolysis">Pyrolysis Conditions</div>', unsafe_allow_html=True)
        for feature in feature_categories["Pyrolysis Conditions"]:
            value = st.session_state.feature_values.get(feature, default_values[feature])
            step = 1.0 if "FT" in feature or "HR" in feature or "FR" in feature else 0.001
            features[feature] = st.number_input(
                feature, value=float(value), step=step, format="%.3f", key=f"pyr_{feature}"
            )
    ...
    with col2:
        reset_clicked = st.button("🔄 重置数据", use_container_width=True)
...
with col_right:
    ...
    st.markdown('<h4>预测结果</h4>', unsafe_allow_html=True)
    if st.session_state.prediction_result is not None:
        st.markdown(f'<div class="result-value">{st.session_state.selected_model}: {st.session_state.prediction_result:.2f} wt%</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div>等待预测...</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    
    # 预测信息
    st.markdown('<div class="info-section">', unsafe_allow_html=True)
    st.markdown('<h4>预测信息</h4>', unsafe_allow_html=True)
    result_text = f"{st.session_state.prediction_result:.4f} wt%" if st.session_state.prediction_result else "未预测"
    st.markdown(f"""
    ...
    # 模型状态
    st.markdown('<div class="info-section">', unsafe_allow_html=True)
    st.markdown('<h4>模型状态</h4>', unsafe_allow_html=True)
    st.markdown(f"""
    ...
    if st.session_state.warnings:
        st.markdown('<div class="info-section">', unsafe_allow_html=True)
        st.markdown('<h4>⚠️ 输入警告</h4>', unsafe_allow_html=True)
